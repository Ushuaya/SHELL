...
{
    if ((f_word == NULL) || (*f_word == '\0') || (*f_word == ' ') || (*f_word == '\n'))
    {
        return NULL_CMD;
    }
    else
    {
        if (f_word[0] != '>'  &&  f_word[0] != '|' && f_word[0] != '&' && f_word[0] != ';' && f_word[0] != '<' && f_word[0] != ' ' && f_word[0] != '\n' &&  f_word[0] != '|')
        {
            return CONV;
        }
        else
        ...
        switch (our_state)
        {
            ...
            case CONV:
                ...
                if (list != NULL)
                {
                    ...
                    if (what_is_this_arg (list[i+1]) != NULL_CMD)
                    {
                       // printf("\n__DEBUG__CONVv_1_   %s\n", list[i]);
                        if ((our_state = what_is_this_arg(list[i])) == CONV)
                        {
            ...
            case CONV1:
                ...
                if((our_state = what_is_this_arg(list[i])) != CONV)
                {
                    ...
                    //free(tmp_tree);
                    fprintf(stderr, "Expected name of command \"<cmd> | <NULL>\"\n");
                    our_state = CIDIOT;
                    goto ERROR_GO;
                }
                //printf("\n__DEBUG__TREE_0_   %s\n", list[i]);
                ...
                while ((our_state = what_is_this_arg(list[i])) == CONV)
                {
                    tmp_tree->argv[j] = list[i];
                    i++; j++;
                }
                j = 1;
            ...
            if ((our_state = what_is_this_arg(list[i])) != NULL_CMD)
            {
                    if((our_state = what_is_this_arg(list[i])) == OUT)
                    {
                        tmp_tree->outfile = list[i+1];
                        i++; i++;
                    }
                    
                    if((our_state = what_is_this_arg(list[i])) == OUT_APP)
                    {
                        tmp_tree->outfile = list[i+1];
                        tmp_tree->append = 1;
                        i++; i++;
                    ...
                    if((our_state = what_is_this_arg(list[i])) == IN)
                    {
                        tmp_tree->infile = list[i+1];
                        i++; i++;
                    }
            }
            ...
            case IN:
                ...
                {
                    //if there is no command after <
                    fprintf(stderr, "Expected name of file \"<cmd> < <NULL>\"\n");
                    our_state = CIDIOT;
                    break;
                }
            ...
            case POINTCOMMA:
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        //cmd;;
                        our_state = CIDIOT;
                        fprintf(stderr, "Don't know command \";<cmd>\"\n");
                        break;
                    }
                }
            case AND_CMD:
                ...
                else
                {
                    //if there is no command after &&
                    our_state = CIDIOT;
                    fprintf(stderr, "Expected command \"<cmd>&&<NULL>\"\n");
                    break;
                }
...
void delete_tree (tree current_cmd)
  {
  if (current_cmd == NULL) return;
  else
    {
 
    //free (current_cmd -> argv);

    delete_tree (current_cmd -> pipe);
    delete_tree (current_cmd -> next);
